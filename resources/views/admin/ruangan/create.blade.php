<div class="modal fade" id="form_create" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document">
        <form method="POST" action="{{ route('admin.ruangan.store') }}" id="bt_submit_create" novalidate>
            @csrf
                                ...
                                <select name="lantai_id" id="lantai_id"
                                    ...
                                    @foreach ($lantais as $lantai)
                                        <option value="{{ $lantai->id_lantai }}"
                                            {{ old('lantai_id') == $lantai->id_lantai ? 'selected' : '' }}>
                                ...
                                <select name="jenis_ruangan" id="jenis_ruangan"
                                    ...
                                    @foreach ($fungsiRuangans as $fungsi)
                                        <option value="{{ $fungsi->id_fungsi }}"
                                            {{ old('jenis_ruangan') == $fungsi->id_fungsi ? 'selected' : '' }}>
                                            {{ $fungsi->nama_fungsi }}
                                        </option>
                                    @endforeach
                                </select>
