...
class PimpinanDashboardController extends Controller
{
    public function index()
    {
        ...
        if (!empty($allKategoriIds)) {
            $kategoriData = Kategori::whereIn('id_kategori', $allKategoriIds)
                ...
                ->map(function ($kat) use ($kategoriCountsRaw) {
                    return ['id' => $kat->id_kategori, 'nama' => $kat->nama_kategori, 'total' => $kategoriCountsRaw[$kat->id_kategori] ?? 0];
                })
                ->groupBy('nama')
                ->map(function ($items) {
        ...
        return view('pimpinan.dashboard.index', compact('stats', 'user', 'kategoriData', 'subKategoriData', 'tipePelapor', 'privasiData', 'bulanData'));
    }
